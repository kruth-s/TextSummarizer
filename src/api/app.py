...
from utils.pdf_generator import save_summary_as_pdf
...
def generate_summary():
    ...
    summary = summarize_text(text)  # Call the summarization logic
    pdf_path = save_summary_as_pdf(summary)

    return {"summary": summary, "pdf_url": "/download_pdf"}
...
def summarize_and_generate_pdf(text):
    response = requests.post("http://127.0.0.1:8000/generate_summary", json={"text": text})
    if response.status_code == 200:
        summary = response.json()["summary"]
        pdf_url = "http://127.0.0.1:8000/download_pdf"
        return summary, pdf_url
    return "Error generating summary", ""

iface = gr.Interface(
    fn=summarize_and_generate_pdf,
    inputs="text",
    outputs=["text", "text"],
    title="GenAI Text Summarizer",
    live=True,
